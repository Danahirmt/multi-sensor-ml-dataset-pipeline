...
def plot_gaps(gaps_ms, thr_ms, title, outpath):
    if gaps_ms.size == 0:
        ...
        plt.title(title)
        plt.text(0.5, 0.5, "No gaps (0/1 sample)", ha="center")
        plt.axis("off")
    ...
    plt.title(
        title
        + f" | median={np.median(gaps_ms):.2f} ms, p95={np.percentile(gaps_ms,95):.2f} ms, max={gaps_ms.max():.2f} ms"
    )
    plt.tight_layout()
...
def plot_timeline(lidar_segs, cam_segs, common_segs, outpath):
    ...
    all_segs = (
        lidar_segs + cam_segs + common_segs
        if (lidar_segs or cam_segs or common_segs)
        else []
    )
    if not all_segs:
        plt.figure(figsize=(12, 2))
        plt.title("No segments")
        plt.axis("off")
        plt.savefig(outpath, dpi=150)
        plt.close()
        return
...
def main():
    ...
    ap.add_argument(
        "--min-length-s", type=float, default=40.0
    )  # not used in plotting, but useful
    ...
    for w in read_ros2_messages(args.mcap, topics=[LIDAR_TOPIC, CAM_TOPIC]):
        t = (
            header_stamp_ns(get_ros_message(w))
            if args.time_source == "sensor"
            else get_log_time_ns(w)
        )
        if t is None:
            continue
        topic = get_topic(w)
    ...
    # Gaps (ms)
    gL = (
        np.diff(np.array(lidar, dtype=np.int64)) / 1e6
        if len(lidar) > 1
        else np.array([])
    )
    ...
    plot_gaps(
        gL,
        args.max_gap_ms,
        f"LiDAR gaps ({args.time_source} time)",
        os.path.join(args.out_dir, "lidar_gaps.png"),
    )
    plot_gaps(
        gC,
        args.max_gap_ms,
        f"Camera gaps ({args.time_source} time)",
        os.path.join(args.out_dir, "camera_gaps.png"),
    )
    plot_timeline(
        segL, segC, common, os.path.join(args.out_dir, "segments_timeline.png")
    )
